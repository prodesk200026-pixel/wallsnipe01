...
import { runIndicatorsAllTimeframes } from '../indicators/registry.js';
 ...
 */
export function runEngineChain(symbolStore, candleStore) {
  ...
  // Computed upstream by server.js's option-chain poller and stored
  // on the snapshot (chain.maxPain) — but never actually pulled into
  // this pipeline's output before now, so it silently never reached
  // the frontend despite being calculated every 20s.
  const maxPain = latestOptionSnapshot?.maxPain ?? { status: 'UNAVAILABLE', reason: 'no option chain snapshot in store' };
  ...
  // Runs every registered indicator (RSI, MACD, and whatever's
  // added to indicators/registry.js later) across every timeframe,
  // on real candles built from the live tick stream. `candleStore`
  // is optional so this pipeline still works in contexts that don't
  // have one wired up (e.g. a future unit test) — indicators simply
  // aren't included in that case, rather than crashing.
  const indicators = candleStore ? runIndicatorsAllTimeframes(candleStore, symbolStore.symbol) : null;
