 ...
 */
export function attachSocket(httpServer, { adapterManager, rollingStore, runEngineChain, candleStore }) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('subscribe:symbol', (symbol) => {
      ...
      socket.emit('engine:update', runEngineChain(store, candleStore));
    });
  ...
  setInterval(() => {
    ...
    for (const symbol of rollingStore.allSymbols()) {
      ...
      const output = runEngineChain(store, candleStore);
