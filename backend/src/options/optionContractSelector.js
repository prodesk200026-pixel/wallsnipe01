...
function scoreContract(c, spot) {
  ...
  return {
    ...
    oi: leg.oi ?? null,
    iv: leg.iv ?? null,
    // Previously computed upstream (server.js's option-chain poller)
    // but dropped here before reaching the frontend — greeks/maxPain
    // were calculated every 20s and then thrown away right before
    // the one place (the entry card) that could actually show them.
    greeks: leg.greeks ?? null,
    oiChange: leg.oiChange ?? null,
    volumeChange: leg.volumeChange ?? null,
    oiChangePct: leg.oiChangePct ?? null,
