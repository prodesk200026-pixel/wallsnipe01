...
export class AdapterManager {
  ...
  _scheduleRetry() {
    ...
    // Exponential backoff (capped) instead of a fixed 15s — a fixed
    // fast retry is exactly what fed Dhan's token rate limit and
    // very likely triggered its anti-abuse WS termination in the
    // first place. Backing off after repeated failures is kinder to
    // Dhan's API and avoids ever hammering it again. Resets to the
    // base interval on the next successful connect (see _activate).
    this._retryAttempt = (this._retryAttempt || 0) + 1;
