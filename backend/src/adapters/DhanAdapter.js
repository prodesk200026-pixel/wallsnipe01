...
import { getUnderlyingConfig } from '../data/instrumentResolver.js';
...
export class DhanAdapter extends BaseMarketDataAdapter {
  constructor({ clientId, accessToken }) {
    ...
    this._intentionalDisconnect = false; // true only when WE call disconnect() — lets 'close' tell intentional teardown apart from Dhan dropping us
  ...
  async connect() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.on('close', (code, reasonBuf) => {
        ...
        // CRITICAL: a close with no preceding 'error' event is common
        // — e.g. Dhan accepts the WS handshake (fires 'open') then
        // closes it moments later because the token/Data-API
        // entitlement check failed server-side, with no error frame
        // at all. Without emitting here, AdapterManager never finds
        // out the connection died and never retries — it just sits
        // on DISCONNECTED forever. So: always emit, unless WE closed
        // it on purpose via disconnect().
        ...
        if (!opened) reject(new Error(`DHAN: WebSocket closed before opening (code ${code})`));
      });
    });
  }
  ...
  /**
   * Dhan's option chain endpoints (both /optionchain and
   * /optionchain/expirylist) are documented at 1 request per 3
   * seconds — slower than most of Dhan's other REST endpoints,
   * because OI data updates slowly server-side. Same category of
   * mistake as the token-generation rate limit fixed earlier:
   * enforce this locally so nothing in this codebase can ever
   * accidentally hammer it faster than Dhan allows.
   */
  async _optionChainRateLimit() {
    const MIN_INTERVAL_MS = 3200; // padded slightly over Dhan's stated 3s
    const now = Date.now();
    const wait = (this._lastOptionChainCallAt || 0) + MIN_INTERVAL_MS - now;
    if (wait > 0) await new Promise((r) => setTimeout(r, wait));
    this._lastOptionChainCallAt = Date.now();
  }
  ...
  /**
   * Accepts a plain symbol string ("NIFTY") — resolves the Dhan
   * securityId/segment via InstrumentResolver, fetches the real
   * nearest expiry from Dhan's own expirylist endpoint (previously
   * this was called with the literal string 'NEAREST_WEEKLY', which
   * Dhan's API does not accept — it needs an actual date), then
   * fetches the chain. Also respects Dhan's documented 1-request-
   * per-3-seconds limit on both of these endpoints (separate from,
   * and in addition to, the token-generation rate limit fixed
   * earlier) — same lesson, applied here before it caused the same
   * kind of silent failure.
   */
  async getOptionChain(symbol, expiryOverride) {
    const cfg = getUnderlyingConfig(symbol);
    const underlyingScrip = Number(cfg.spotReference.securityId);
    const underlyingSeg = cfg.spotReference.exchangeSegment;

    let expiry = expiryOverride && expiryOverride !== 'NEAREST_WEEKLY' ? expiryOverride : null;
    if (!expiry) {
      await this._optionChainRateLimit();
      const expiryRes = await this.http.post('/optionchain/expirylist', {
        UnderlyingScrip: underlyingScrip,
        UnderlyingSeg: underlyingSeg
      });
      expiry = expiryRes.data?.data?.[0] || null;
